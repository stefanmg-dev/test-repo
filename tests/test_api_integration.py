...
def test_extract_document_endpoint(monkeypatch):
    monkeypatch.setattr(
        routes_extract,
        "extract_text",
        fake_extract_text
    ...
    assert response_body["document_type"] == "invoice"
    assert response_body["raw_text"] == A1_RAW_TEXT
    assert response_body["llm_values"] == {}
    assert response_body["final_values"] == EXPECTED_VALUES
...
"""
    ...
    async def fake_invalid_extract_text(file):
        await file.read()
        return invalid_raw_text

    monkeypatch.setattr(
        routes_extract,
        "extract_text",
        fake_invalid_extract_text
    )
    ...
    assert response_body["validation"]["valid"] is False

    errors = response_body["validation"]["errors"]
